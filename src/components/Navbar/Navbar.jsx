          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) => 
                  `text-white hover:text-orange font-medium transition-colors relative group ${
        ...
        <nav className="flex flex-col h-full pt-8 px-8">
          {navLinks.map((link) => (
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) =>
                `text-white/80 hover:text-orange py-4 text-xl font-medium border-b border-orange/10 ${
                  isActive ? 'text-orange' : ''
                }`
              }
